...
| `/gh-issue-driven:goal <milestone> [flags]` | G | **Autonomously finish a whole milestone**: for each open issue, run `start` → implement → `/code-review` → PR → assign-based Copilot loop → session-summary, checkpointing between issues. Runs unattended through green/yellow verdicts; only a **red** verdict pauses for HITL. Never merges, never pushes to the default branch. Flags: `dry-run`, `force`, `resume`. |
