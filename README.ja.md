...
| `/gh-issue-driven:goal <milestone> [flags]` | G | **マイルストーンを自律的に完遂**: 各 open issue について `start` → 実装 → `/code-review` → PR → assign ベース Copilot ループ → session-summary を回し、issue 間で state を checkpoint。green/yellow は無停止で進み、**red のみ** HITL で停止。マージも default branch への push もしません。フラグ: `dry-run`, `force`, `resume` |
